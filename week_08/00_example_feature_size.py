...
X = iris.data[:, 0:1] # only using the 'sepal_length' variable
y = iris.target

# giving me 4 new arrays; splitting our sample into testing and training
from sklearn.model_selection import train_test_split
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2,
                                                    random_state=0) # random state is setting a seed
...
logR.fit(X_train_std, y_train) # fit on the training dataset
print(logR.score(X_test_std, y_test)) # gets the accuracy score .. not very good

# .score() == same as doing predict(X_test) and comparing to y_test --> gives us accuracy score
...
cv = StratifiedKFold() # stratfied = tries to put an equal number of each targets in each fold

X = iris.data[:, 0:4] # choosing features of the data
y = iris.target
...
scores = cross_val_score(logR, X_std, y, cv=cv) # using our logistic regression model (don't need to standardize y, but it's just 1 or 0, so no need)
