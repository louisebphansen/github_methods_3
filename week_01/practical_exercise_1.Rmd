...
lm_model <- lm(V4 ~ V2 + V3, data = design_matrix)
summary(lm_model)
```
...
- We have plotted a line with slope 1 and intercept 1 to show the difference between the model predictions and the actual y-values. If the two values were the same, they would all be on the green line. 
...
cubic_model <- lm(V5 ~ V2 + V3 + V4, data = cub_df)
summary(cubic_model)
...
cubic_betahat[4]
```
 - The estimated value of the "cubic" parameter is 0.0459, meaning that it is very small. It doesn't add much to our model.
...
mean(mtcars$mpg)
```
It's identical to the mean, since making a regression with only an intercept is the same as taking the mean of the dependent variable. 
...
ggplot(logistic_df, aes(x=V2, y=yhat_logistic))+ 
  geom_point()
```
...
mtcars_plot <- ggplot(logistic_df, aes(x = V2, y = V1))+
  geom_point()+
  geom_text(aes(label=ifelse(V1 != predicted, names, '')),hjust=0,vjust=0)+
...
#this is how far as i got.... i couldn't get it to filter out the wrong guesses in the plot..

nrow(filter(logistic_df,logistic_df$V1 != logistic_df$predicted))

#but we can see that the prediction is wrong three times.
```
...
- Not a lot - the predicted values are almost the same
...
- the logistic model has the lower AIC, thus providing the better fit. The residual deviance is also a bit higher for the quadratic fit - but not a lot. This means that the quadratic term does not improve the model.
