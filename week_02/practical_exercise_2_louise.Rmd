...
```{r}
#finding the residual standard deviation and AIC, and combining into a tibble for better overview
...
AIC_values <- AIC(gender_model, lmer_gender_model_scenario, lmer_gender_model_subject, lmer_gender_model_both)

as_tibble(cbind(models, sigmas, AIC_values))
```

The model with both random intercepts for subject and scenario has the lowest residual standard deviation and lowest AIC value.
...
```{r}
#this allows me to plot the two qq-plots next to each other
par(mfrow = c(1,2))

qqnorm(resid(gender_model), main = "QQplot,residuals of gender_model")
qqline(resid(gender_model))

qqnorm(resid(avg_model), main = "QQplot, residuals of avg_model")
qqline(resid(avg_model))
```


I would argue that the second model, shown in the second plot, fulfills the assumption of normality of the residuals of the general linear model better. This is because the errors look more systematically distributed in the first plot; the first plot seems to be a bit right-skewed. Furthermore, it seems as though more of the errors are distributed above the line, whereas in the second plot, they are more evenly distributed both above and under. However, there are naturally fewer observations in the second plot, as this contains averaged values. Since these values are averaged, the residuals naturally resembles a normal distribution more.
...
It looks all right - there is not as much of a pattern as we saw in the first plot above. The errors are more evenly distributed above and under the line in this plot - but there are still some deviations from the line, which we can see to the far right of the plot. This shows that the residuals are still a bit right-skewed
...
The slope for "attitudepol" is -17.19. This is the slope for women in the polite condition. This means that compared to the intercept, which is the average frequency for women in the informal condition, the pitch decreases with -17.19 Hz in the polite condition compared to the informal condition. If we look at the interaction coefficient, which is 5.54, we can add this to the slope of "attitudepol", to get the effect on pitch frequency in the polite condition for men. This gives us a slope of -11.648, which means that compared to the informal condition, politeness decreases the pitch frequency with -11.648 Hz for men. This is is less steep than for women, which we can interpret as women's frequency being more affected by politeness than men's. However, we see that the error bars are overlapping quite a lot, so there could be some uncertainty about whether this is interaction is a real effect. 
...
residual_variances <- c(sum((resid(lmer_gender_model_both)^2)), sum((resid(attitude_model)^2)), sum((resid(interaction_model)^2)))
...
```
The first model, with only gender as a fixed effect and random intercepts for subject and scenario, has the highest value on all three parameters. The interaction model has the lowest residual variance and lowest AIC value. However, model with gender and attitude as fixed effects (as well as random intercepts for subject and scenario) has the lowest residual standard deviation. But overall, there is not that big of a difference between the two models with attitude as a fixed effect. However, we choose the model with the lowest residual standard deviation; this is because residual standard deviation, sigma, takes the number of parameters of the model into account, and thus penalizes models with many parameters, which residual variance does not; this measure will always give the lowest value to the model with the most parameters, thus making it a biased measured for measuring variance explained. 
